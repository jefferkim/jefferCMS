...
function dataProcess(data)
{
	...
	for (rIndex=0; rIndex<recordLength; rIndex++)
	{
		...
		if(rIndex%2==0){
					var row = $('<tr><td align="center" valign="middle" rel="chk"></td><td height="30" align="center" valign="middle" rel="id"></td><td align="left" style="padding-left:10px;" valign="middle" rel="username"></td><td align="left" style="padding-left:10px;" valign="middle" rel="company"></td><td align="center" valign="middle" rel="logintime"></td><td align="center" valign="middle" rel="islock"></td><td align="center" valign="middle"><a  href="javascript:;" onclick="Edit_id('+recordList[rIndex].ID+')"><img src="../images/33.gif" width="14" height="14" class="img_ico">修改</a> <a href="javascript:;" onclick="Del_id('+recordList[rIndex].ID+')"><img src="../images/11.gif" width="14" height="14" class="img_ico">删除</a> <a href="sys_usersave.php?action=addlen&id='+recordList[rIndex].ID+'"><img src="../images/daoru.gif" width="14" height="14" class="img_ico"/>导入语言模板</a></td></tr>');
		}else{
		...
		row.find("td[rel=username]").html(recordList[rIndex].USERNAME);
		row.find("td[rel=company]").html(recordList[rIndex].USERCALLED);
		row.find("td[rel=company]").html(recordList[rIndex].COMPANY);
		row.find("td[rel=logintime]").html(recordList[rIndex].LOGINTIME);
...
function showDetail(id) {
	AjaxGetCHTML("sys_user.json.php",$.param({id:id}),function(data)
	{
		$(".dragDiv2").html(data);									 
	});
...
$(function()
{
	...
	$("#btnEdit").click(function()
	{
		...
		if (checkedVal != "")
		{
			//window.open(,'memberedit','top=0,left=0,scrollbars=1,width='+ $(window).width() +',height='+$(window).height());
			window.location.href = 'sys_useredit.php?id='+checkedVal;
		}
	});
	...
	$("#btnDelete").click(function()
	{
		...
		if (checkedVal != "")
		{
			AjaxSetConfirm("确定要删除吗？","sys_userset.php",$.param({action:'del',id:checkedVal}),function(data)
			{
				AjaxGet(g_url,getParameter(),dataProcess);
			});
		}
	...
	$("#btnLock").click(function()
	{
		...
		if (checkedVal != "")
		{
			AjaxSetConfirm("确定要锁定吗？","sys_userset.php",$.param({action:'lock',id:checkedVal}),function()
			{
				AjaxGet(g_url,getParameter(),dataProcess);
			});
		}
	...
	$("#btnUnLock").click(function()
	{
		...
		if (checkedVal != "")
		{
			AjaxSetConfirm("确定要解除锁定吗？","sys_userset.php",$.param({action:'unlock',id:checkedVal}),function()
			{
				AjaxGet(g_url,getParameter(),dataProcess);
			});
		}
...
function Del_id(id){
	...
	AjaxSetConfirm("确定要删除吗？","sys_userset.php",$.param({action:'del',id:id}),function()
			{
				AjaxGet(g_url,getParameter(),dataProcess);
			});
...
function Edit_id(id){
window.open('sys_useredit.php?id='+id,'edit','top=0,left=0,scrollbars=1,width='+ $(window).width() +',height='+$(window).height());
};
