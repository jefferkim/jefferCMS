...
$pictypeRs = $SysConfig['customerdb']->Execute("SELECT Called FROM t_protype where id='".$type."'");
if ($pictypeRs->RecordCount() >0)
{
	$typename = $pictypeRs->fields['Called'];
}		 
			
$proRs = $SysConfig['customerdb']->Execute("SELECT OrderBy,id FROM t_products ORDER BY OrderBy DESC LIMIT 0,1");
$count=count($proRs);
if ($proRs->RecordCount() >0)
{
	$orderBy = $proRs->fields['OrderBy'] + 1;
	$proid=$proRs->fields['id'] + 1;
}else{
	$orderBy=1;
	$proid=1;
}

$content="<img src='".globalpath()."/upload/".$filename."'/>";
	...
	$SysConfig['customerdb']->AutoExecute("t_products",$record,"INSERT");
