...
$contentRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_custom2 WHERE id=?",array($contentId));
...
$fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_custom2' ORDER BY id");
while (!$fieldRs->EOF)
{
...
$text_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_custom2' and UiType='textarea' ORDER BY id");
...
$deltext_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_custom2' and UiType<>'textarea' ORDER BY id");
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" class="main_bj">
  ...
  <tr>
    ...
    <span style="font-weight:bold">位置：</span>系统自定义功能 — <?=$custom2?>修改
   </div>
