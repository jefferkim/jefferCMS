...
class NewsDao extends BaseDao
{
	...
	//添加新闻
	function add($title,$content,$isCommend)
	{
		...
		$this->logger->add(GetIPAddr(),"添加系统公告，ID:".$id.",标题:".$title);

		return $id;
	}

	//修改新闻
	function Edit($id,$title,$content,$userName,$isShow,$isCommend,$orderBy,$newsType,$language,$parent,$smallPic,$customerFieldArr)
	{
		$record['Title'] = stripslashes($title);
		$record['Content'] = stripslashes($content);
