<?
//ppg product type dao
//zjb
//080722

include_once("ppgproductsdao.class.php");

class PpgProductTypeDao extends BaseDao
{
	var $logger;

	function PpgProductTypeDao($db)
	{
		$this->db = $db;
		$this->table = 't_ppgprotype';

		$this->logger = new Logger($db);
	}

	//添加一个产品分类
	function Add($called,$pid,$language,$isShow,$orderBy,$memo)
	{
		$typeLevel = 0;
		$parentPath = "";
		...
		$record = $this->GetRecord($called,$pid,$language,$isShow,$orderBy,$memo);
	...
	//修改产品分类
	function Edit($id,$called,$pid,$language,$isShow,$orderBy,$memo)
	{
		$record = $this->GetRecord($called,$pid,$language,$isShow,$orderBy,$memo);
	...
	function Delete($id,$cascade=false)
	{
		...
		if ($cascade)
		{
			$productDao = new PpgProductsDao($this->db);
	...
	function GetRecord($called,$pid,$language,$isShow,$orderBy,$memo)
	{
