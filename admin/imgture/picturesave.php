...
include_once("../dao/imgturedao.class.php");
...
function add($db)
{
	...
	$fieldRs = $db->Execute("SELECT * FROM t_fields WHERE TypeName='t_img' ORDER BY id");
	while (!$fieldRs->EOF)
	{
	...
	{
		if ($language[$i] != "")
			$dao->Add($called,$spic,$bpic,$isshow,0,$proid,$language[$i],$customerFieldArr);
	}
...
function edit($db)
{
	...
	$IsCommend = $db->Execute("SELECT IsCommend from t_img WHERE id=?",array($id));
	...
	$dao = new PictureDao($db);
	$dao->Edit($id,$called,$spic,$bpic,$isshow,$cid,$proid,$language,$customerFieldArr);
