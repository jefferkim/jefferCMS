...
$(function()
{
	$("#btnSave").click(function()
	{
		...
		AjaxSet("productimportsave.php",q,function(data)
		{
			alert(data['result']);
			if (data['result']=='导入成功')
			{
	...
	/*$("select[name=outputLan]").change(function()
	{
		var q = $.param({language:$(this).val()});
		AjaxGet("typelanlist.json.php",q,function(data)
		{
			$("select[name=outputType]").empty();
			$("select[name=outputType]").append('<option value="">全部</option>');
			for (i=0; i<data.length; i++)
			{
				$("select[name=outputType]").append('<option value="'+ data[i]['ID'] +'">'+ data[i]['CALLED'] +'</option>');
			}
		});
	});

	$("select[name=importLan]").change(function()
	{
		var q = $.param({language:$(this).val()});
		AjaxGet("typelanlist.json.php",q,function(data)
		{
			$("select[name=importType]").empty();
			for (i=0; i<data.length; i++)
			{
				$("select[name=importType]").append('<option value="'+ data[i]['ID'] +'">'+ data[i]['CALLED'] +'</option>');
			}
		});
	});*/
})
