...
$contentRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_content WHERE id=?",array($contentId));
...
$fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_content' ORDER BY id");
while (!$fieldRs->EOF)
{
...
$text_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_content' and UiType='textarea' ORDER BY id");
...
$deltext_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_content' and UiType<>'textarea' ORDER BY id");
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" class="main_bj">
  ...
  <tr>
    ...
    <span style="font-weight:bold">位置：</span>基础信息管理 — 内容信息修改
   </div>
