...
$editMode = true;
$id = $_REQUEST['id'];
$rs = $SysConfig['customerdb']->Execute("SELECT * FROM t_downloadtype WHERE id=?",array($id));
$obj = $rs->FetchObject();
$called = $obj->CALLED;
...
$fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_downloadtype' ORDER BY id");
while (!$fieldRs->EOF)
{
...
$text_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_downloadtype' and UiType='textarea' ORDER BY id");
...
$deltext_fieldRs = $SysConfig['customerdb']->Execute("SELECT * FROM t_fields WHERE TypeName='t_downloadtype' and UiType<>'textarea' ORDER BY id");
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" class="main_bj">
  ...
  <tr>
    ...
    <span style="font-weight:bold">位置：</span>下载信息管理 — 下载分类修改
   </div>
